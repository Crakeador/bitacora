...
while($c = $q->fetch_array(MYSQLI_ASSOC)){
    echo '</br>Ubicacion: '.$c["descripcion"].' Agente: '.$c["name"].'</br>';
	...
	while($a = $r->fetch_array(MYSQLI_ASSOC)){
		...
		if($a['id'] ==  3 && $c['hadicional'] == 1){ // Horas suplementarias
			if($c['cargo'] == 7){
			    $horas = ($diasla * 4)+($nochel * 4);
                $valor = ($horas*(465.06/240))*1.5;
                
		        $formula = '('.$horas.'*(465.06/240))*1.5';
			}else{
				$valor = 0;
				$formula = '';
			}
		...
		if($a['id'] ==  4){ // Fondos de Reserva
			if($c['decimo'] == 2){
				...
				//Formula: sueldo * 8.33%
				$valor = ($calcular*8.33)/100;
				$formula = '('.$calcular.'*8.33)/100';
			}else{				
				$valor = 0;
				$formula = '';
			}
		...
		if($a['id'] ==  6){ // Decimo 4to.
			if($c['acumula4'] == 0){
				...
				$valor = (460/360)*$totals; // $base_4to/12; //($a['valor']+$suma)
				$formula = '(460/360)*'.$totals;
			}else{
				$valor = 0;
				$formula = '';				
			}
		...
		if($a['id'] == 12){ // IESS
			$valor = ($calcular*$a['valor'])/100; //$base_3ero
        ...
        if($a['id'] == 24 && $c['hnocturna'] == 1){ // Horas Nocturnas
            if($c['cargo'] == 7){
                $diarios = ($nochel * 8);
                $valor= ($diarios*(465.06/240))*0.25;
                
    	        $formula = '('.$diarios.'*(465.06/240))*0.25';
			}else{
				$valor = 0;
				$formula = '';
			}
