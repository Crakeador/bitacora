...
class DescuentoData {
	...
	static function getById($id){
		$sql = "SELECT * FROM ".self::$tablename." WHERE depart = ".$_SESSION['depart']." AND idperson = $id AND is_active = 1"; //echo $sql;
		$query = Executor::doit($sql);
		return Model::one($query[0],new DescuentoData());
	}

	public static function getByPerson($id){		
		$sql = "SELECT * FROM ".self::$tablename." WHERE idperson = $id AND is_active = 1"; //echo $sql;
		$query = Executor::doit($sql);
