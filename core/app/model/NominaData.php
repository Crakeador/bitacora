...
class NominaData {
	...
	public static function getAll($client, $id, $ano, $mes){
		$sql = "SELECT C.id, B.tipo_cuenta, C.idcard, C.name, C.fechanacimiento, C.startwork, D.description, A.monto 
				 ...
				 WHERE A.person = $id and A.mes = $mes and A.ano = $ano and A.rubro = B.id and A.person = C.id and C.idcargo = D.id AND A.client = $client";
	...
	public static function getAllMonto($ano, $mes, $tipo){
		$sql = "SELECT A.*, B.tipo_cuenta from nomina A, rubro B, person C, cargo D WHERE A.mes = $mes and A.ano = $ano and A.rubro = B.id and A.person = C.id and C.idcargo = D.id and D.idtipo = $tipo and A.company = ".$_SESSION['id_company'];  
		$query = Executor::doit($sql);
	...
	public static function getHistorico($ano, $mes, $client){
		$sql = "SELECT A.mes, A.ano, F.idclient, F.idclient idservicio, F.descripcion puesto, A.client, C.id, C.idcard, C.name, C.fechanacimiento, C.startwork, D.description, A.monto 
				 ...
				 WHERE A.person = C.id AND C.idcargo = D.id AND A.client = F.id AND 
	...
	public static function getAllTotal($ano, $mes, $tipo, $client){
		$sql = "SELECT A.mes, A.ano, F.idclient, F.idclient idservicio, F.descripcion puesto, A.client, C.id, C.idcard, C.name, C.fechanacimiento, C.startwork, D.description, A.monto 
				  FROM nomina A, person C, cargo D, puestos F 
				 WHERE A.person = C.id AND C.idcargo = D.id AND A.client = F.id AND C.is_active = 1 AND
	...
	public static function getById($ano, $mes, $id, $cliente){
		...
		$sql .= "where A.client = $cliente AND A.person = $id and A.mes = $mes and A.ano = $ano and A.rubro = B.id and A.person = C.id and C.idcargo = D.id ";
