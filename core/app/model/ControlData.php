...
class ControlData {
	...
	public static function getCentrol($id){ 
		$sql = "SELECT B.nombre, A.* FROM ".self::$tablename." A, client B WHERE A.idclient = B.id AND A.idclient = $id";
	    $query = Executor::doit($sql);
	...
	public static function getAll(){
		$sql = "SELECT B.nombre, A.* FROM ".self::$tablename." A, client B WHERE A.idclient = B.id";
		$query = Executor::doit($sql);
