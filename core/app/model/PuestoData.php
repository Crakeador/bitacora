...
class PuestoData {
	...
	public static function getZonas($id){
		$sql = "SELECT * FROM zonas WHERE idpuesto = $id AND is_active = 1";
		$query = Executor::doit($sql); 
		return Model::many($query[0],new PuestoData());
	}
	...
	public static function getByIdHorario($id, $tipo, $estado, $ini=null, $fin=null){
		...
		$sql .=  "FROM personpuestos A, puestos B, person C, cargo D ";
		$sql .= "WHERE A.idservicio = B.id AND A.idperson = C.id AND C.cargo = D.id AND A.idservicio = $id AND D.idtipo IN ($tipo) AND A.is_active = $estado ";
		if($ini == NULL)
			$sql .= "";
		else
			$sql .= "AND date(C.startwork) >= \"".$ini."\" AND date(C.startwork) <= \"".$fin."\" ";
	...
	public static function getByIdDesvinculado($id, $tipo, $estado){
		...
		$sql .=  "FROM personpuestos A, puestos B, person C, cargo D ";
		$sql .= "WHERE A.idservicio = B.id AND A.idperson = C.id AND C.cargo = D.id AND A.idservicio = $id AND D.idtipo = $tipo AND A.is_active = $estado ";
