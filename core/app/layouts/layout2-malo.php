...
$url = 'https://latin.near-solution.com/'; $_SESSION['url'] = 'https://latin.near-solution.com/';
if(isset($_SESSION['depart'])) $departamento = DepartamentoData::getById($_SESSION['depart']);

if(isset($_SESSION["user_id"])){
	$notificcion = TimelineData::getAllByUserId($_SESSION["user_id"], date("m"), date("Y"), 1);
  ...
  if(isset($_SESSION["cambio"])){
    ...
    if($diferencia->format('%R%a') > 260) $notificacion++;
  }
}
//var_dump($_SESSION);
?>
...
<html lang="es">
  <head>    
    ...
    <meta name="description" content="Puedes tener el control de tu negocio con nuestro modulos desarrollados en casos reales de los diferentes negocios en el Ecuador" />
    <title>CIPOL | Dashboard</title>
    <meta property="og:url" content="http://cipol.ec">
    <meta property="og:title" content="CIPOL Soft ERP">
    <link rel="icon" type="image/jpg" href="<?php echo $url; ?>assets/images/icon-service.png">
    <?php // Seleccion de los temas del Sistema 
        if(!isset($_SESSION["user_id"])){ ?>
            <!-- DashForge CSS -->
            <link type="text/css" rel="stylesheet" href="<?php echo $url; ?>assets/css/dashforge.css"/>
            <link type="text/css" rel="stylesheet" href="<?php echo $url; ?>assets/css/dashforge.auth.css"/>
            <!-- Alertas del Sistema -->
            <link type="text/css" rel="stylesheet" href="<?php echo $url; ?>plugins/sweetalert/sweetalert.css"/> <?php
        }else{ 
          // Theme style
    ...
    <style>
		...
		.loader {
			...
			align-items: center;
			font-size: 2rem;
			background: url('https://cipol.ec/sidai/assets/images/cargando.gif') 50% 50% no-repeat rgb(249,249,249);
			opacity: .8;
		}
    ...
    <!-- jquery-2.2.4 -->
    <script type="text/javascript" src="<?php echo $url; ?>plugins/jQuery/jquery.min.js"></script>
    <script type="text/javascript" src="<?php echo $url; ?>plugins/jQueryUI/jquery-ui.js"></script>
	<script type="text/javascript" src="<?php echo $url; ?>assets/js/jquery.flexslider.js"></script>
    <!-- Switchery -->
    <script type="text/javascript" src="<?php echo $url; ?>plugins/switchery/switchery.min.js"></script>
    <!-- Sweet Alert -->
    <script type="text/javascript" src="<?php echo $url; ?>plugins/sweetalert/sweetalert.min.js"></script>
    <script type="text/javascript"><?php 
        if($_SESSION['idrol'] == 12){ ?>
            function esAntesDeLas17Horas() {
                const ahora = new Date();
                return ahora.getHours() < 17;
            }
  ...
  <body id="sidai" class="<?php if(isset($_SESSION["user_id"]) || isset($_SESSION["client_id"])){ echo "sidebar-mini skin-black sidebar-collapse fixed"; } else { echo ""; } ?>" onload="cerrar();">
        ...
        <div class="wrapper">
          ...
          <?php if(isset($_SESSION["user_id"]) || isset($_SESSION["client_id"])):?>
            ...
            <footer class="main-footer">
              ...
              <strong>Copyright &copy; 2016-2020 <a href="https://cipol.ec/" target="_blank">CIPOL Soft ERP</a></strong>
            </footer>
              <!-- Control Sidebar -->
    ...
    <?php else: ?>
      ...
      <footer class="footer">
            <div>
              <span>&copy; 2020 Bitacora <span>El&eacute;ctronica</span> v3.0 </span>
              <span>Created by <a href="https://cipol.ec">CIPOL Soft</a></span>
            </div>
            <div>
              <nav class="nav">
                <a href="https://cipol.ec/licenses/standard" class="nav-link">Licencia</a>
                <a href="https://cipol.ec/help" class="nav-link">Ayuda</a>
              </nav>
            </div>
        </footer>
    ...
    <!-- Sweet Alert -->
    <script type="text/javascript" src="./plugins/sweetalert/sweetalert.min.js?v=1.0.1"></script>
    <?php
        if(isset($_GET["error"])){
            if($_GET["error"] == 1) $mensaje = '- El usuario no existe';
            if($_GET["error"] == 2) $mensaje = '- El guardia no tiene puesto asignado';
            if($_GET["error"] == 3) $mensaje = '- La clave esta errada, corrija';
            if($_GET["error"] == 4) $mensaje = '- Su clave esta errada';
            if($_GET["error"] == 5) $mensaje = '- Su clave esta errada';
        ...
        if(isset($_GET["view"])){
			...
			if($_GET["view"]=="newprovider" || $_GET["view"]=="editprovider" || $_GET["view"]=="catres.residents" || $_GET["view"]=="autorizar" || $_GET["view"]=="novedad" || 
			   $_GET["view"]=="catcli.clients" || $_GET["view"]=="corizar" || $_GET["view"]=="listadiaria" || $_GET["view"]=="empleado"){
			...
			if($_GET["view"]=="asignar" || $_GET["view"]=="novedad" || $_GET["view"]=="informe" || $_GET["view"]=="supervisar" || $_GET["view"]=="aspirantes" || 
			   $_GET["view"]=="odometro" || $_GET["view"]=="visita" || $_GET["view"]=="visito" || $_GET["view"]=="puesto" || $_GET["view"]=="custodia" || 
			   $_GET["view"]=="reporte" || $_GET["view"]=="ronda" || $_GET["view"]=="registro" || $_GET["view"]=="visitar"){
                //if(isset($_GET["view"]) &&  $_GET["view"]=="custodia") echo 'setInterval("location.reload()", 30000);'; else echo '';
                if(!isset($_SESSION["user_id"])){
                  // Eliminada la Session
                }else{
                    echo '<script type="text/javascript">
                            ...
                            function errorCallback(error) {
                              ...
                              if (error.core == error.PERMISSION_DENIED) {
                                ...
                                appErrMessage = "GPS Desconectado"
                              } else {
    ...
    <script type="text/javascript">
        ...
        $(function () {
          <?php
            if(isset($_GET["view"])){ 
                if($_GET["view"]=="newprovider" || $_GET["view"]=="editprovider" || $_GET["view"]=="catres.residents" || $_GET["view"]=="autorizar" || $_GET["view"]=="novedad" || $_GET["view"]=="catcli.clients"){
    ...
    <?php
      if(isset($_GET["view"])):
         if($_GET["view"]=="home" || $_GET["view"]=="inventary" || $_GET["view"]=="cuentas" || $_GET["view"]=="bitacora" || $_GET["view"]=="carnets" || $_GET["view"]=="memos" ||
            $_GET["view"]=="fechas" || $_GET["view"]=="partes" || $_GET["view"]=="prendas" || $_GET["view"]=="descuento" || $_GET["view"]=="visitas" || $_GET["view"]=="archivos" ||
            $_GET["view"]=="autorizan" || $_GET["view"]=="autorizo" || $_GET["view"]=="verificados" || $_GET["view"]=="cotizacion" || $_GET["view"]=="visitantes" ||
            $_GET["view"]=="departamento" || $_GET["view"]=="categorias" || $_GET["view"]=="productos" || $_GET["view"]=="informes" || $_GET["view"]=="opehor.clientes" || 
            $_GET["view"]=="articulos" || $_GET["view"]=="faltas" || $_GET["view"]=="clientes" || $_GET["view"]=="telefonos" || $_GET["view"]=="rondas" || $_GET["view"]=="familia" ||
            $_GET["view"]=="rrphor.activos" || $_GET["view"]=="rrphor.inactivos" || $_GET["view"]=="conducta" || $_GET["view"]=="providers" || $_GET["view"]=="cartera" || 
            $_GET["view"]=="reportsbyclient" || $_GET["view"]=="listadiaria" || $_GET["view"]=="novedad" || $_GET["view"]=="oficios" || $_GET["view"]=="agentes" ||  
            $_GET["view"]=="dotar" || $_GET["view"]=="puestos" || $_GET["view"]=="vehiculos" || $_GET["view"]=="entregas" || $_GET["view"]=="recibos" || $_GET["view"]=="informar" || 
            $_GET["view"]=="pendientes" || $_GET["view"]=="custodia" || $_GET["view"]=="usuarios" || $_GET["view"]=="puestos" || $_GET["view"]=="novedades" || $_GET["view"]=="aspirantes" ||
            $_GET["view"]=="puesto" || $_GET["view"]=="empleados" || $_GET["view"]=="equipar" || $_GET["view"]=="tareas" || $_GET["view"]=="permisos" || $_GET["view"]=="servicios" ||
            $_GET["view"]=="catres.lista" || $_GET["view"]=="opecor.lista" || $_GET["view"]=="catdes.lista" || $_GET["view"]=="catcar.lista" || 
            $_GET["view"]=="catrol.lista" || $_GET["view"]=="cobnom.lista" || $_GET["view"]=="sisnot.lista" || $_GET["view"]=="catres.lista" || 
            $_GET["view"]=="rrhpre.lista" || $_GET["view"]=="rrhliq.lista" || $_GET["view"]=="rrhmac.lista" || $_GET["view"]=="rrhdoc.lista" ||
            $_GET["view"]=="rrsdoc.lista" || $_GET["view"]=="opeasp.lista" || $_GET["view"]=="opeasi.lista" || $_GET["view"]=="catrub.lista" ||
            $_GET["view"]=="sisaud.lista" || $_GET["view"]=="rrging.lista" || $_GET["view"]=="rrping.lista" || $_GET["view"]=="rrhvac.lista" ||
            $_GET["view"]=="rrsing.lista" || $_GET["view"]=="catlim.lista" || $_GET["view"]=="repent.lista" || $_GET["view"]=="catofi.lista" ||
            $_GET["view"]=="catloc.lista"): ?>
            <!-- DataTables --> 
            ...
            <script src="<?php echo $url; ?>plugins/datatables/extensions/Responsive/js/dataTables.responsive.min.js"></script>
            <!-- Productos
