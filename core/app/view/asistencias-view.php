...
if(isset($_GET['valor'])){
    ...
    if($_SESSION['dispositivo'] == 1) $cadena = " AND fecha BETWEEN '".date("Y-m-d", strtotime("-30 day", strtotime($hoy)))." 00:00:00' AND '".date("Y-m-d", strtotime("+1 day", strtotime($hoy)))." 00:00:00'";
...
$users = AsistenciaData::getAll($cadena);
                            ...
                            <tbody>
  								<?php
									...
									foreach($users as $tables){
										if($tables->foto == '') $cadena = 'storage/persons/american.png'; else $cadena = 'storage/horario/'.$tables->foto;
										if($tables->status == 1)
											$turno = 'Ingreso';
										else
											$turno = 'Salida';
