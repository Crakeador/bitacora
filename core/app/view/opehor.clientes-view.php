<?php
// Modulo de Operaciones para el registro del horaio de los agentes activos
/* Modificacion para los clientes
if($_SESSION["idrol"] == "6")
	$puestos = PuestoData::getAllLugar(2);
else
	$puestos = PuestoData::getAll(2); */

$puestos = ClientData::getTotal(); 
$ano=date("Y");
//var_dump($puestos);
	...
	<div class="col-xs-12">
		<div class="box">		
			<div class="box-header with-border"> 
				<div class="col-md-4 col-sm-10">
					<div class="dataTables_length" id="example_length">
						<label>Ver el reporte de asistencia de:&nbsp;
							<select style="width: 120px; display: inline-block;" id="mes_id" name="mes_id" class="form-control" onchange="javascript:location.href='?view=opehor.clientes&mes='+value;">
				...
				<div class="col-md-7 col-sm-10">					
					<div id="example_filter">
						...
						<label>
							<?php
								echo '<select id="localidad_id" name="localidad_id" class="form-control" onchange="javascript:location.href=\'index.php?view=opehor.clientes&id=\'+value;">';
								echo '<option value="0"> -- SELECCIONE PUESTO -- </option>';
								foreach($puestos as $tables) {
									if($tables->idclient > 1){
										if($tables->idclient == $lugar) $valor = 'selected'; else $valor = '';
										echo '<option value="'.$tables->idclient.'" '.$valor.'>'.$tables->nombre.'</option>';
									}	
								}
								echo '</select>';
							?>
			...
			<div class="box-body mailbox-messages">
				...
				<div style="height: 650px; width: 100%; overflow-x: auto;" id="example_length">
					<?php
						// $users = PuestoData::getByIdHorario($lugar, 3, 1);  Original
						$users = PuestoData::getByIdCliente($lugar, 3, 1);

						if(count($users) > 0){	?>
							<table id="viewlista" class="table table-bordered table-hover">
								<thead>
									<tr>
										<th style="width:60%"><div align="center">Horario</div></th>
									</tr>
								</thead>
								<tbody>
									<?php									
