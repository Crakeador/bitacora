<?php
/*
* Modulo de Operaciones para el registro del horaio de los colaboradores activos
*/
if(isset($_GET['id'])){
	$lugar=$_GET['id'];
	$_SESSION['id']=$lugar;
...
<section class="content-header">
	<h1>
		...
		<small>registro de la asistencia del personal administrativo</small>
	</h1>
	<ol class="breadcrumb">
	...
	<div class="box">		
		<div class="box-header with-border"> 
			<div class="col-md-6 col-sm-10">
				<div class="dataTables_length" id="example_length">
					<label>Ver el reporte de asistencia de:&nbsp;
						<select style="width: 120px; display: inline-block;" id="mes_id" name="mes_id" class="form-control" onchange="javascript:location.href='index.php?action=horario&tipo=1&mes='+value;">
			...
			<div class="col-md-6 col-sm-10">					
				<div id="example_filter">
					...
					<label>
						<?php
							echo '<select id="localidad_id" name="localidad_id" class="form-control" onchange="javascript:location.href=\'index.php?action=horario&tipo=1&id=\'+value;">';
		...
		<div class="box-body mailbox-messages">
				...
				<div class="col-sm-12">
					<div class="alert alert-dismissable alert-info">
						...
						<p> - Los turnos se identifican con los siguientes colores:&nbsp;</br>
							...
							<span class="label label-danger" >F</span>&nbsp;Falta y
			...
			<div style="height: 410px; width: 100%; overflow-x: auto;" id="example_length">
				<?php
					$users = PuestoData::getByAdministrador($lugar);
					if(count($users) > 0){
						echo '<table id="horario" class="table table-bordered table-hover">';
							echo '<tbody>';
							// Crea tabla del Personal Administrativo
											...
											for ($i = 1; $i <= $total; $i++) {
												echo '<td>';
													if($resultado > 0) {
														...
														echo '<input type="hidden" id="hiden_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'" name="hiden_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'" value="'.$iddia.'-'.$tables['servicio'].'-'.$tables['id'].'-'.str_pad($i, 2, "0", STR_PAD_LEFT).'-'.$mes.'-'.$ano.'-0">';
														$clase = 'btn-default';
													}
													echo '<input type="button" id="button_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'" name="button_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'" class="btn '.$clase.'" onClick="btn_NuevoOnClick('.$iddia.', '.str_pad($i, 2, "0", STR_PAD_LEFT).', '.$mes.', '.$ano.','.$tables['id'].','.$tables['servicio'].',\'button_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'\',\'hiden_'.$tables['servicio'].'_'.$tables['id'].'_'.$i.'\');" value="'.str_pad($i, 2, "0", STR_PAD_LEFT).'"/>';
												echo '</td>';
											}
											echo '</tr>';
										echo '</table>';
...
<script>
	...
	document.title = "Near Solutions | Horario Administrativos"
