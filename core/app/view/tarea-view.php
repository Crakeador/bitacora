...
if(count($_POST)>0){
	...
	if(isset($_GET) && isset($_GET["id"])){
		...
		$fecha = explode(" ",$tarea->date_event);
		$tarea->date_event = $fecha[0];
	}else{
...
<section class="content-header">
	...
	<ol class="breadcrumb">
		<li><a href="<?php if($_SESSION["is_admin"] == 1) echo $_SESSION["url"].'tareas'; else echo $_SESSION["url"].'home'; ?>"><?php if($_SESSION["is_admin"] == 1) echo '<i class="fa fa-database"></i> Tareas'; else echo '<i class="fa fa-dashboard"></i> Inicio'; ?> </a></li>
		<li class="active"> Asignaci&oacute;n</li>
	</ol>
</section>
<section id="main" role="main" style="padding: 1.5rem !important;">	
	<form class="form-horizontal" method="post" id="addconducta" name="addconducta" action="<?php echo $_SESSION["url"]; ?>tarea" role="form">
		<input type="hidden" id="tarea_id" name="tarea_id" value="<?php echo $_SESSION["idtarea"]; ?>">
		<div class="callout callout-danger" style="margin-bottom: 0!important;">
			<button type="submit" class="btn btn-success pull-right" <?php if(isset($_SESSION["idtarea"]) && $_SESSION["idtarea"] > 0) echo 'style="display:none;"'; else echo ''; ?>><span class="glyphicon glyphicon-floppy-disk"></span> Guardar </button>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-md-4 col-sm-4">
						<div class="input-group date form_date col-md-2 col-sm-6" data-date-format="yyyy-mm-dd">
							<input type="date" class="form-control" id="ini_fec" name="ini_fec" value="<?php echo $tarea->date_event; ?>" required title="Debe de ser una fecha válida">
						</div>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-md-4">
						<textarea class="form-control input-sm" id="id_descripcion" name="descripcion" rows="4" required placeholder="Ingrese la descripción de la tarea"><?php echo $tarea->title; ?></textarea>
					</div>
				</div>		
			</div>
		</div>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title">Descripci&oacute;n de la Tarea</h4>
								</div>
							...
							<table id="viewBitacora" class="table table-bordered table-hover">
								<thead>
									<tr>
										...
										<th><div align="center">DESCRIPCION DE LA TAREA</div></th>
									</tr>
								</thead>
								<tbody>
									<?php
									if($_SESSION["idtarea"] == NULL){
										...
										if($resultado > 0){
											foreach($users as $tables) {
												echo '<tr>';
													echo '<td><div align="center">'.$i.'&nbsp;&nbsp;<a href="index.php?view=tareas&borrar='.$tables->id.'&id='.$_SESSION['idtarea'].'" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i></a></div></td>';
													echo '<td>'.$tables->fecha.'</td>';
