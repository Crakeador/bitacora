<?php $sell = SellData::getByIdPerson($_GET["id"]); // print_r($sell); ?>
...
<section class="content-header">
	...
	<ol class="breadcrumb">
		<li><a href="./index.php?view=repent.lista"><i class="fa fa-book"></i> Talento Humano </a></li>
...
<section class="content" style="padding: 1.5rem !important;">
  ...
  if(isset($_GET["id"]) && $_GET["id"]!=""):
    // var_dump($sell);
    $productos = $sell->getByIdProduct($sell->id);
  ...
  if($sell->person_id!=""):
    ...
    echo '<tr>';
      ...
      echo '<td>'.utf8_encode($client->idcard).'-'.utf8_encode($client->name).'</td>';
    echo '</tr>';
  endif;
  if($sell->user_id!=""):
  $user = $sell->getUser();
  ...
  <tr>
      <td>Atendido por</td>
      <td><?php echo utf8_encode($user->name)." ".utf8_encode($user->lastname);?></td>
  </tr>
  <?php endif; ?>
  </table>
  <br>
...
<script type='text/javascript'><!--
	function btn_Imprimir($id) {
		VentanaCentrada('documentos/puestos_pdf.php?id='+<?php echo $_SESSION['id_venta']; ?>,'Entrega de Dotacion','','1024','768','true');
	}
</script>