...
URL_PLACA_ALT = "https://terminalterrestredeguayaquil.com/consultar-propietario-de-vehiculo/"
...
def consultar_placa(placa: str) -> Tuple[str, bool]:
    """
    Flujo combinado y optimizado (2–3s de espera):
      1) Terminal Terrestre de Guayaquil (consultar-propietario-de-vehiculo): extrae
         Placa, Dueño, Marca, Modelo, Clase, Año, Servicio, País.
      2) EPMTSD (vehiculo_seguro): obtiene SOLO ESTADO (ROBADO / NO ROBADO),
         o "ESTADO: Página en mantenimiento o no disponible" si no responde.

    Devuelve: (texto_formateado, robado_bool)
    """
    import unicodedata, time as _time, re as _re
    from bs4 import BeautifulSoup
    from selenium.webdriver.common.by import By
    from selenium.webdriver.common.keys import Keys
    ...
    try:
        ...
        # ===============================
        # 1) Terminal Terrestre (propietario)
        # ===============================
        datos: dict[str, str] = {}
        try:
            driver.get(URL_PLACA_ALT)
            _time.sleep(2)

            # Localizar input robustamente (placeholder Ej: ABC1234)
            input_sel_list = [
                "input[name='placa']",
                "input[id*='placa' i]",
            ...
            if not input_box:
                raise Exception("No se encontró el campo de placa en el Terminal Terrestre")

            input_box.clear()
            input_box.send_keys(placa)

            # Localizar botón "Consultar Datos"
            btn = None
            btn_xpaths = [
                "//button[contains(., 'Consultar Datos')]",
                "//button[contains(., 'Consultar')]",
                "//input[@type='submit' or @value='Consultar']"
            ]
            ...
            _time.sleep(3)  # espera optimizada

            soup = BeautifulSoup(driver.page_source, "html.parser")

            # Intento 1: pares etiqueta-valor por coincidencia exacta de etiqueta
            etiquetas = ["Placa", "Dueño", "Marca", "Modelo", "Clase", "Año", "Servicio", "País", "Color"]
            def get_value_for_label(lbl: str):
                # Busca un nodo cuyo texto visible coincida exactamente con la etiqueta
                lab = soup.find(lambda t: getattr(t, "get_text", None) and _norm(t.get_text(strip=True)) == _norm(lbl))
                if not lab:
                    return None
                # Intenta: hermano siguiente con texto distinto a la etiqueta
                sib = lab.find_next_sibling()
                if sib and getattr(sib, "get_text", None):
                    txt = sib.get_text(" ", strip=True).strip()
                    if txt and _norm(txt) != _norm(lbl):
                        return txt
                # Si no, toma los textos de la "fila" contenedora y usa lo que sigue
                row = lab.find_parent(['li', 'tr', 'div'])
                if row:
                    # hijos directos comunes
                    parts = [c.get_text(" ", strip=True) for c in row.find_all(['div','span','td','small','b','strong'], recursive=False)]
                    parts = [p for p in parts if p]
                    if parts:
                        # ubica el label y devuelve lo que sigue
                        for i, p in enumerate(parts):
                            if _norm(p) == _norm(lbl):
                                val = " ".join(parts[i+1:]).strip()
                                if val and _norm(val) != _norm(lbl):
                                    return val
                # fallback: próximo nodo con texto distinto
                nxt = lab.find_next(lambda t: getattr(t, "get_text", None) and t.get_text(strip=True))
                if nxt and _norm(nxt.get_text(strip=True)) != _norm(lbl):
                    return nxt.get_text(" ", strip=True).strip()
                return None

            for lab in etiquetas:
                v = get_value_for_label(lab)
                if v:
                    key = "DUEÑO" if _norm(lab) in ("DUENO","DUEÑO") else ("PAÍS" if _norm(lab) in ("PAIS","PAÍS") else lab.upper())
                    datos[key] = v

            lineas.append(f"PLACA {placa} (navegador: {used})")
            # Orden de salida esperado por tu interfaz
            for campo in ("DUEÑO","MARCA","MODELO","CLASE","AÑO","SERVICIO","PAÍS","COLOR"):
                val = datos.get(campo)
                if val:
                    lineas.append(f"{campo}: {val}")

        except Exception as e:
            lineas.append(f"PLACA {placa} (navegador: {used})")
            lineas.append(f"ERROR Terminal Terrestre: {e}")

        # ===============================
        # 2) EPMTSD: solo ESTADO
        # ===============================
        try:
            ...
            if not input_placa:
                lineas.append("ESTADO: Página en mantenimiento o no disponible")
            else:
                input_placa.clear()
                ...
                texto = _norm(soup2.get_text())

                if any(x in texto for x in ("NO DISPONIBLE","MANTENIMIENTO","ERROR SERVIDOR")):
                    lineas.append("ESTADO: Página en mantenimiento o no disponible")
                elif ("ROBADO" in texto) or ("REPORTADO" in texto):
                    # Buscar negaciones claras
                    if _re.search(r"(NO\s+ROBADO|NO\s+REPORTADO|SIN\s+REPORTE|NO\s+REGISTRA)", texto):
                        lineas.append("ESTADO: NO ROBADO")
                        robado = False
                    else:
                        lineas.append("ESTADO: ROBADO")
                        robado = True
                else:
                    lineas.append("ESTADO: Página en mantenimiento o no disponible")
...
<div style="max-width:400px;margin:4rem auto;">
  <div class="card">
    ...
    <div class="card-body">
      \1
<div class=\"form-group\"><label>Buscar Cédula o Placa</label><input class=\"form-control\" name=\"query_search\" placeholder=\"Ej: 0952364897 o PCE3214\"></div>
...
def _filter_rows_by_query(rows, query_search):
    try:
        q = (query_search or "").strip()
    except Exception:
        q = ""
    if not q:
        return rows
    ...
    out = []
    for r in rows or []:
        v = r.get("query") or r.get("consulta") or ""
        if isinstance(v, str) and v.strip().upper() == qn:
            out.append(r)
    return out
...
def historial(request: Request, server_id: str = "", tipo: str = "", estado: str = ""):
    ...
    # New: optional single-query filter
    try:
        form = request.form() if hasattr(request, "form") else {}
    except Exception:
        form = {}
    query_search = (form.get("query_search") or request.query_params.get("query_search") or "").strip()
    u, r, allowed = require_login(request)
    rows = DATA.list_for_user(u, r, allowed)
    rows = _filter_rows_by_query(rows, query_search)
    if server_id:
...
def export_get(request: Request):
    ...
    # New: optional single-query filter for export
    try:
        form = request.form() if hasattr(request, "form") else {}
    except Exception:
        form = {}
